...
Languages:: The concept of languages is deeply embedded into the system's language _Chora_. If you want to caret a multi-lingual app, you can do so by smply adding more languages here. 
...
* Identify the item _Text_, representing a simple text view. Click on the element, drag it over to the root canves and place it in the middle of the canvas.
...
* Now have a look at the https://doc.embedded-wizard.de/inspector-window[inspector windows^] right to the main composer window: in the upper _member area_ you should see the element named text selected. Also note the attributes and properties area below that shows all properties of the currently selected text view.
* Inside the attributes and properties area, we can adapt the newly inserted text view to our needs:
** Using the dropdown list, alter the _Font_ property of the text elemnt to the value _Resources::FontExtraLarge_.
